...
class Net(nn.Module):
    def __init__(self, STATE_NUM, ACTION_NUM):
        ...
        self.fc1 = nn.Linear(in_features=STATE_NUM, out_features=10)  # 全连接层1
        self.fc2 = nn.Linear(in_features=10, out_features=ACTION_NUM)  # 全连接层2
...
class DQN(object):
    ...
    def learn(self):
        ...
        q_eval = self.eval_net(batch_state).gather(1, batch_action)  # 对应位置取数据
        q_next = self.eval_net(batch_next_state).detach()  # 分离一个variable
        q_target = batch_reward + GAMMA * \
            q_next.max(1)[0].view(BATCH_SIZE, 1)  # 使用q_next的最大reward值来推荐动作
...
for episode in range(EPISODE_NUM):
    ...
    while True:
        ...
        action = dqn.choose_action(state)  # 选择action epsilon-greedy 策略
        ...
        dqn.save_transition(state, action, next_state, reward)  # 存储转移

        total_reward += reward
        if dqn.position > MEMORY_CAPACITY:  # 仅有memory满载时才学习
            dqn.learn()
            if is_done:
