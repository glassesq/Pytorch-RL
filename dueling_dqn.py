...
for episode in range(EPISODE_NUM):
    ...
    while True:
        ...
        action = duelingdqn.choose_action(state)  # 选择action epsilon-greedy 策略
        ...
        duelingdqn.save_transition(state, action, next_state, reward)  # 存储转移

        total_reward += reward
        if duelingdqn.position > MEMORY_CAPACITY:  # 仅有memory满载时才学习
            duelingdqn.learn()
